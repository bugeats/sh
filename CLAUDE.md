...
Config modules come in two shapes: data modules (`starship.nix`, `git.nix`, `jj.nix`, `gitui.nix`) take the flake's `configInputs` record (colors, identity) and return pure content, TOML-shaped ones as attrsets rendered by `pkgs.formats.toml`; directory modules (`fish/`, `zellij/`) encapsulate config assembly via `default.nix`. The `zellij/` directory concatenates static KDL (`config.kdl`), generated keybinds (loop in `default.nix`), and generated theme (`theme.nix`), and writes `bar.nix` to `layouts/bar.kdl`, selected by `default_layout`. `layout_dir` is appended at build time as the absolute store path (why: comment in `zellij/default.nix`); the session manager's new-session layout picker cannot be disabled, but the default layout sorts first so Enter picks it. That layout replaces the built-in tab-bar with [zjstatus](https://github.com/dj95/zjstatus) (flake input; the wasm builds from source, about a minute on first build per machine): one left-aligned line of mode glyph, `hostname -s`, session name, then the tabs; nothing on the right, no bottom bar. zjstatus prompts for permissions in the bar line on first launch and again whenever its store path changes; press `y`, the grant persists in `~/.cache/zellij/permissions.kdl`. zellij does not tilde-expand `config.kdl` paths; a nonexistent `default_cwd` breaks the plugin's `run_command`. Keybinds are documented in [docs/zellij-keybinds.md](docs/zellij-keybinds.md).
